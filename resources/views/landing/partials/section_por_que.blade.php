...
<section id="por-que-bromovil" class="py-20 bg-slate-50">
          ...
          <div class="reveal feature-card group" style="--delay: {{ 60 * $i }}ms">
            ...
            <div class="relative z-[1] p-3 md:p-3.5">
              <div class="aspect-square grid place-items-center rounded-xl bg-white border border-slate-200/70 logo-pill">
                ...
                @else
                  {{-- Fallback vector con gradiente corporativo --}}
                  @switch($b['key'])
                    @case('comisiones')
              ...
              <div class="mt-2 text-[11px] md:text-[12px] font-medium text-slate-700 text-center truncate">
                {{ $b['label'] }}
              </div>
  ...
  <style>
    ...
    /* Tarjeta pro (sutil, sin exagerar) */
    .feature-card{
      position:relative;border-radius:16px;overflow:hidden;background:#fff;
      border:1px solid rgba(15,23,42,.08);
      box-shadow:0 6px 18px rgba(2,6,23,.04);
      transition:transform .2s ease, box-shadow .2s ease, border-color .2s ease, background .2s ease;
      animation:fcIn .4s ease forwards;animation-delay:var(--delay,0ms)
    }
    .feature-card:hover{
      transform:translateY(-4px);
      box-shadow:0 14px 28px rgba(65,156,246,.10), 0 8px 20px rgba(132,79,240,.08);
      border-color:rgba(65,156,246,.22);
      background:linear-gradient(180deg, #ffffff 0%, #f8fafc 100%);
    }

    /* Aros/glow sutiles corporativos */
    .feature-card__ring{
      position:absolute;inset:-1px;border-radius:18px;
      background:conic-gradient(from 180deg at 50% 50%, rgba(65,156,246,.0), rgba(132,79,240,.18), rgba(65,156,246,.0));
      filter:blur(12px);opacity:0;transition:opacity .25s ease;
    }
    .feature-card__glow{
      position:absolute;inset:auto -14px -14px auto;width:80px;height:80px;border-radius:9999px;
      background:radial-gradient(closest-side, rgba(132,79,240,.22), rgba(255,255,255,0));
    ...
    /* Logos (SVG/PNG) – gris a color */
    .logo-img{
      max-height:30px; max-width:68%; object-fit:contain;
    ...
    @media (prefers-reduced-motion: reduce){
      .reveal, .feature-card{transition:none;animation:none}
