...
<script>
  ...
  (function(){
    ...
    const cart = {
      ...
      add(sku, title, price, qty=1, img=null){
        price = Number(price)||0; qty = Number(qty)||1;
        const i = this.items.findIndex(x=>x.sku===sku);
        if(i>-1){
          this.items[i].qty += qty;
          if (img && !this.items[i].img) this.items[i].img = img;
        } else {
          this.items.push({sku:String(sku), title:String(title), price:Number(price), qty:Number(qty), img: img || null});
        }
        this.save();
      },
    ...
    function open(){ if (overlay) overlay.hidden=false; if (drawer){ drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); } }
    function close(){ if (overlay) overlay.hidden=true; if (drawer){ drawer.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); } }
    ...
    function wire(){
      ...
      // Botones de "Agregar al carrito" (dashboard + tienda)
      const hookAddButtons = () => {
        $$('.js-add-cart, .add-to-cart, .js-buy-now').forEach(btn=>{
          ...
          btn.addEventListener('click', (ev)=>{
            ...
            const img   = btn.dataset.img  || btn.getAttribute('data-img')   || null;
            cart.add(String(sku), String(title), Number(price), Number(qty), img);
            open();
          });
      ...
      // Rehook dinámico si cambias la vista vía Turbo/Livewire (opcional)
      document.addEventListener('bm:rehook-cart', hookAddButtons);
    }

    cart.load(); render(); wire();
    window.BM_CART = {
      add:(sku,title,price,qty=1,img=null)=>{ cart.add(sku,title,price,qty,img); },
  ...
  (function(root){
    ...
    function render(){
      if (el.outSims) el.outSims.textContent = state.sims;
      if (el.sims) el.sims.value = state.sims;
      if (el.porta) el.porta.value = state.porta;
      if (el.recargas) el.recargas.value = state.recargas;
      if (el.monto) el.monto.value = state.monto;
      if (el.outResidualBadge) el.outResidualBadge.textContent = state.doble ? '8%' : '4%';
      if (el.outGanPlan) el.outGanPlan.textContent = fmt2(PLANS[state.plan].ganancia);
      const r = calc();
      if (el.outVenta)    el.outVenta.textContent    = fmt2(r.venta);
      if (el.outResidual) el.outResidual.textContent = fmt2(r.residual);
      if (el.outSipab)    el.outSipab.textContent    = fmt2(r.sipab);
      if (el.outTotal)    el.outTotal.textContent    = fmt2(r.total);
      reflectChips();
    }
    ...
    el.chips.forEach(ch=>{
      ch.addEventListener('click', ()=>{
        ...
        if(bind==='plan'){ state.plan = val; state.monto = PLANS[state.plan].sugMonto; if (el.monto) el.monto.value = state.monto; }
    ...
    ['input','change'].forEach(evt=>{
      el.sims    && el.sims.addEventListener(evt, e=>{ state.sims = Math.max(0, +e.target.value||0); render(); });
      el.porta   && el.porta.addEventListener(evt, e=>{ state.porta = +e.target.value||0; render(); });
      el.recargas&& el.recargas.addEventListener(evt, e=>{ state.recargas = +e.target.value||0; render(); });
      el.doble   && el.doble.addEventListener(evt, e=>{ state.doble = !!e.target.checked; render(); });
      el.monto   && el.monto.addEventListener(evt, e=>{ state.monto = +e.target.value||0; render(); });
    });
  ...
  (function(){
    ...
    function setRange(kind){
      ...
      if (from) from.value = d1? fmt(d1) : '';
      if (to)   to.value   = d2? fmt(d2) : '';
    }
